...
public class AddAvailableSkin implements CommandExecutor {

    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (commandSender.hasPermission("skinsitemssystem.addavailableskin")) {
            if (strings.length == 3) {
                String playerName = strings[0];
                String itemName = strings[1];
                String skinName = strings[2];

                UserDataManager.addAvailableSkin(playerName, itemName, skinName);
                return true;
            }
        } else {
